  ...
  standingsAsTable: computed(
    ...
    function () {
      return this.get('model.standings')
        .toArray()
        .map((s) => {
          const championBet = this.get('model.championBets').find(
            (cB) => cB.get('user.id') === s.get('user.id')
          );

          if (championBet != null) {
            s.set('championBet', championBet);
            s.set(
              'hasCorrectChampionBet',
              this.get('model.competition.champion.id') ===
                championBet.get('team.id')
            );
          }
        ...
        .map((value, index, arr) => {
          value.set(
            ...
            index === 0
              ? 1
              : this.compareStandings(value, arr[index - 1]) > 0
              ? index + 1
              : null
          );
  ...
  championBetOfAuthenticatedUser: computed(
    ...
    function () {
      return this.get('model.championBets').find(
        (cB) => cB.get('user.id') === this.get('model.authenticatedUser.id')
      );
    }
  ),
});
