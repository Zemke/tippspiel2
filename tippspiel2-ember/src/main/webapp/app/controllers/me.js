...
export default Controller.extend({
  ...
  actions: {
    ...
    manuallySetChampion(championTeam) {
      ...
      if (!confirm(this.get('intl').t('me.manualChampion.confirm', {teamName}))) {
        return;
      }

      this.set('model.currentCompetition.champion', championTeam);
      this.get('model.currentCompetition').save()
        .then(() => this.get('resHandler').handleSuccess('me.manualChampion.success'))
        .catch(err => this.get('resHandler').handleError(err))
    }
  }
});
